...
// Invincibility frames  (120 frames ≈ 2 s at 60 fps)
bool  isInvincible    = false;
float invincTimer     = 0.0f;
const float INVINC_DURATION = 45.0f;   // shorter cooldown so obstacles damage consistently
...
// Score & high-score
int score      = 0;   // total score in current run
int levelScore = 0;   // score earned inside the current level only
int hiScore    = 0;   // displayed best score for the current level

const int MAX_GAME_LEVELS = 5;
int hiScoreByLevel[MAX_GAME_LEVELS + 1] = {0};   // index 1..5 used

// Animated clouds (Level 1)
...
// Next-level button (on win end-screen)
...
float starPhase[STAR_COUNT];   // individual blink offset
float starTime5 = 0.0f;        // accumulated frame counter
...
// Forward declaration so keyPress() can call update()
...
void drawPauseScreen() {
    ...
    glColor4f(0.1f, 0.1f, 0.3f, 0.90f);
    glBegin(GL_QUADS);
        glVertex2f(-3.5f, -2.0f); glVertex2f(3.5f, -2.0f);
        glVertex2f(3.5f,  2.5f);  glVertex2f(-3.5f, 2.5f);
    glEnd();
    ...
    drawText(-1.6f, 1.5f, "~ PAUSED ~");

    glColor3f(0.8f, 0.95f, 1.0f);
    drawText(-2.5f, 0.4f, "Press  P  to resume");

    char buf[64];
    sprintf(buf, "Score: %d", score);
    glColor3f(0.7f, 1.0f, 0.7f);
    drawText(-1.2f, -0.6f, buf);
...
void Doraemon()
{
    ...
    glEnd();
    //MUKHER lal hasir plane edge border
    ...
    glEnd();
    glBegin(GL_POLYGON);//majhe khali chilo seta fill koreci
    glColor3f(1.0f, 1.0f, 1.0f);
    ...
    float J_x = 7.08, J_y = 8.41;   // Point J (major axis reference)
    float K_x = 6.6, K_y = 8.55;  // Point K (minor axis reference)
    ...
    glBegin(GL_LINES);
    glColor3f(0.0f, 0.0f, 0.0f);
    glVertex2f(7.665f, 7.229f);//u
    glVertex2f(7.396f, 7.20f);//v
    glVertex2f(7.396f, 7.20f);
    glVertex2f(7.147f, 7.21f);//w
    glVertex2f(7.147f, 7.21f);
    glVertex2f(7.13f, 7.23f);//z
    glVertex2f(7.13, 7.23f);
    glVertex2f(7.11f, 7.25f);//A1
   ...
   // float pi=3.1416;
    ...
    glColor3f(1.0f, 0.84f, 0.0f);   // golden color (R=1, G=0.84, B=0)
    glBegin(GL_POLYGON);
    for(int i = 0; i < 300; i++) {
    ...
    glBegin(GL_LINES);
    glColor3f(0.0f, 0.0f, 0.0f);
    glVertex2f(7.35f, 5.53f);    // x, y
    glVertex2f(7.18f, 5.45f);    // x, y
    glEnd();
    glLineWidth(1);
    glBegin(GL_LINES);
    glColor3f(0.0f, 0.0f, 0.0f);
    glVertex2f(7.18f, 5.45f);    // x, y
    glVertex2f(7.37f, 5.4f);    // x, y
    glEnd();
